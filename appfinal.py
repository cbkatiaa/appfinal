# -*- coding: utf-8 -*-
"""
Created on Wed Sep 11 19:12:33 2024
...
opcion = st.sidebar.selectbox('Selecciona una opción', ['Reporte', 'Scouting'])

# Pestaña de Reporte
if opcion == 'Reporte':
    st.title('Reporte de Rendimiento')
...
if st.button("Generar Análisis"):
    ...
    st.title('Comparación de Jugadores - Scouting')
    
    # Seleccionar jugadores
    jugadores = st.multiselect('Selecciona Jugadores', df['Player'].unique())
    
    if len(jugadores) > 0:
        df = df[df['Player'].isin(jugadores)]
...
# Crear Radar Chart
    ...
    st.subheader('Comparación de Métricas')
        ...
        # Loop para crear gráficos de barras por cada métrica
    for metric in ['Press %', 'Éxito duelos aéreos', 'Duelos aéreos ganados', 'Acciones agresivas', 'Asistencias Torneo', 'Asistencias Jugada Abierta p90', 'Promedio minutos jugados por partido', 'Pases hacia atrás %']:
            st.write(f"Comparación de {metric}")
            fig, ax = plt.subplots()
            df_jugadores.groupby('Player')[metric].mean().sort_values().plot(kind='barh', ax=ax)
            st.pyplot(fig)
